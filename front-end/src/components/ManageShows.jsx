        ...
        <table className="w-full border-collapse border border-[#e0e0e0]">
          <thead>
            <tr className="bg-[#db0a5b] text-white">
              ...
              <th className="p-3 border border-[#e0e0e0]">Available Seats</th>
              <th className="p-3 border border-[#e0e0e0]">Booked Seats</th>
            </tr>
          </thead>
          <tbody>
            {shows.length === 0 ? (
              <tr>
                <td colSpan="7" className="p-3 text-center text-white">
                  No shows available
                </td>
              </tr>
            ) : (
              shows.map((show, index) => (
                <tr
                  key={show._id}
                  className={`hover:bg-[#f4e4f6] ${index % 2 === 0 ? "bg-[#333]" : "bg-[#444]"}`}
                >
                  ...
                  <td className="p-3 border border-[#e0e0e0] text-white">
                    {show.totalSeats - (show.bookedSeats ? show.bookedSeats.length : 0)} {/* Available seats */}
                  </td>
                  <td className="p-3 border border-[#e0e0e0] text-white">
                    {/* Display booked seats as comma-separated values */}
                    {show.bookedSeats && show.bookedSeats.length > 0
                      ? show.bookedSeats.join(", ")
                      : "No bookings"}
                  </td>
